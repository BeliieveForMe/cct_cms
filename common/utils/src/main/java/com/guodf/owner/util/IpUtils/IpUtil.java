...
public class IpUtil {
	// 用来做为cache，查询一个ip时首先查看cache，以减少不必要的重复查找
		...
		// 定义存储
		private static List<Region> regLst;
		...
		public IpUtil(){
			...
			regLst = new ArrayList<Region>();
		}
		
		
		public IpUtil(RandomAccessFile accessFile) {
			// 纯真库路径
//			this.INSTALL_DIR = ConfigUtil.getValue("config", "ip.dat.dir");
//			log.info("从配置文件获取到的dat路径为" + INSTALL_DIR);
			// 文件名称
//			this.IP_FILE = ConfigUtil.getValue("config", "ip.dat.name");
//			log.info("从配置文件获取到的dat名称为" + IP_FILE);
...
//			}
		...
		public List<IPEntry> getIPEntriesDebug(String s) {
			...
			for (long offset = ipBegin + 4; offset <= endOffset; offset += Constant.IP_RECORD_LENGTH) {
				// 读取结束IP偏移
		...
		public IPLocation getIPLocation(String ip) {
			...
			String tmp = getArea(ip);
			location.setArea(tmp);
		...
		public List<IPEntry> getIPEntries(String s) {
			...
			try {
				...
				for (int offset = (int) ipBegin + 4; offset <= endOffset; offset += Constant.IP_RECORD_LENGTH) {
		...
		private long getMiddleOffset(long begin, long end) {
			long records = (end - begin) / Constant.IP_RECORD_LENGTH;
			records >>= 1;
			if (records == 0)
				records = 1;
			return begin + records * Constant.IP_RECORD_LENGTH;
		}
		...
		private IPLocation getIPLocation(long offset) {
			try {
				...
				} else if (b == Constant.REDIRECT_MODE_2) {
					loc.setCountry(readString(readLong3()));
		...
		private String readArea(long offset) throws IOException {
			ipFile.seek(offset);
			byte b = ipFile.readByte();
			if (b == Constant.REDIRECT_MODE_1 || b == Constant.REDIRECT_MODE_2) {
		...
		public String getCode(String name) {
			try {
				...
				if (StringUtils.isBlank(name)) {
					...
					for (int i = 0; i < Constant.reg.length; i++) {
						flag = name.matches(Constant.reg[i]);
						if (flag) {
							break;
						} else if (!flag) {
							continue;
					...
					if (flag) {
						// 判断数组大小得知是否已经查询数据库
						if (regLst.size() > 0 && !resMap.isEmpty()) {
							// 如果已经加载数据 从Map中循环key值进行相似度计算进行取值
							for (String key : resMap.keySet()) {
								BigDecimal similarity = new BigDecimal(String.valueOf(Util.getSimilarityRatio(name, key)));
							...
							for (String key : resMap.keySet()) {
								...
								// 如果计算出的相似度高于90%，则认为当前数据就是需要获取的信息
								if (retval >= Constant.POINT) {
									return  resMap.get(key).toString().replace("\\|", "");
								} else {
									continue;
								}
							}
				...
				log.error("根据名称获取地市编码发生异常，异常信息为" + e);
			}
			return Constant.RESULT_ERROR_CODE;
		}
		 ...
		 * @return
		 */
		public List<Region> qryRegion() {
			try {
				ApplicationContext factory = new ClassPathXmlApplicationContext(
						"classpath:/springxml/applicationContext_IP.xml");
				/*IpDao ipdao = (IpDao) factory.getBean("ipdao");*/
				/*regLst = ipdao.queryProvincialInfo();*/
				
//				List<Region> municipalityLst = ipdao.queryRegionInfoMunicipality();
//				regLst.addAll(municipalityLst);
			} catch (Exception e) {
				log.error("获取地市数据信息发生异常,异常信息为" + e);
			}
			return regLst;
		}
		 ...
		 * @return
		 */
		public Map<String, Object> lstTransMap(List<Region> lst) {
			try {
				if (lst.size() > 0) {
					// 地址id
					String region = "";
					...
					for (int i = 0; i < lst.size(); i++) {
						// 发现级别为0 省 市 直辖市
						if (Constant.REGION_LEVEL.equals(lst.get(i).getRegion_level())) {
							region = lst.get(i).getRegion_id();
							region_name = lst.get(i).getRegion_name();
						} else {
							String s = lst.get(i).getRegion_id().substring(0, 2);
							if (region.equals(lst.get(i).getRegion_id().substring(0, 2))) {
								resMap.put(region_name + lst.get(i).getRegion_name(),
										region + "_" + lst.get(i).getRegion_id());
							}
						}
					}
				}
			} catch (Exception e) {
				log.error("地市信息转换Map中发生异常，异常信息为" + e);
			}
			return resMap;
		}
		 ...
		 * @return
		 */
		public Map<String, Object> lstTransMapProvincial (List<Region> lst) {
			try {
				if (lst.size() > 0) {
					for (int i = 0; i <lst.size() ; i++) {
						resMap.put(lst.get(i).getRegion_name(), lst.get(i).getRegion_id());
					}
				}
			} catch (Exception e) {
				log.error("转换省会城市信息为Map类型发生异常,异常信息为" + e);
			}
			return resMap;
		}
}
